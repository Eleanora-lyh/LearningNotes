...
你Hive 本质上是一个 **"SQL → MapReduce/Tez/Spark 任务"的编译器 + 元数据管理系统**，它自己**不存储数据**，数据存在 HDFS，**表与文件的映射关系**存在 Metastore（元数据库，通常是 MySQL/Derby）。

![image-20260408225149972](./assets/image-20260408225149972.png)



### 4.1.1 SQL 转 MapReduce 的 6 个阶段（核心流程）
...
| 阶段  | 名称                                          | 做了什么                                                | 产出                |
| --- | ------------------------------------------- | --------------------------------------------------- | ----------------- |
| 1   | **Parser（解析）**                              | 用 Antlr 把 SQL 字符串解析成**抽象语法树 AST**                   | AST Tree          |
| 2   | **Semantic Analyzer（语义分析）**                 | 查 Metastore，校验表/字段是否存在、类型是否匹配                       | QueryBlock（QB）    |
| 3   | **Logical Plan Generator（逻辑计划）**            | 把 QB 转成由 **Operator** 组成的逻辑执行计划                     | OperatorTree（逻辑）  |
| 4   | **Logical Optimizer（逻辑优化）**                 | 谓词下推、列裁剪、分区裁剪、常量折叠等                                 | 优化后的 OperatorTree |
| 5   | **Physical Plan Generator（物理计划）**           | 把逻辑算子翻译成 **MapReduce Task / Tez DAG / Spark Stage** | TaskTree          |
| 6   | **Physical Optimizer + Execution（物理优化+执行）** | Map join 转换、合并小文件等，提交到 YARN 执行                      | MR Job 运行         |
...
```
SELECT dept, COUNT(*) FROM employee WHERE age>30 GROUP BY dept
        │
        ▼ Antlr 词法/语法分析
   AST (TOK_QUERY
          (TOK_FROM employee)
          (TOK_INSERT
            (TOK_SELECT dept count(*))
            (TOK_WHERE (> age 30))
            (TOK_GROUPBY dept)))
```

此时 Hive 会**访问 Metastore**，查询：

- `employee` 这张表存不存在？
- 它的 schema（字段、类型）是什么？
- 它在 HDFS 上的路径是哪里？比如 `/user/hive/warehouse/employee/`
- 它的存储格式是什么？（TextFile / ORC / Parquet）
- 用什么 SerDe 反序列化？

#### 🔹 阶段 3：逻辑算子树（OperatorTree）

Hive 把 SQL 拆成一串 Operator：

```
TableScanOperator  (扫 employee)
        ↓
FilterOperator     (age > 30)
        ↓
SelectOperator     (取 dept 列)
        ↓
GroupByOperator    (按 dept 分组，部分聚合)   ← Map 端
        ↓
ReduceSinkOperator (Shuffle 写出)            ← Map/Reduce 边界
        ↓
GroupByOperator    (最终聚合)                 ← Reduce 端
        ↓
FileSinkOperator   (写结果)
```

> 💡 **关键点**：`ReduceSinkOperator` 就是 Map 和 Reduce 的分界线，它对应 MR 中的 Shuffle 阶段。

#### 🔹 阶段 5：物理计划 → MapReduce

上面那条 SQL 最终翻译成的 MR Job 大致是：

| 阶段               | 工作内容                                                                                                                                              |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| **InputFormat**  | 根据 Metastore 里的路径 `/user/hive/warehouse/employee/`，按 `HiveInputFormat` 切分文件                                                                       |
| **Map 任务**       | 1) 读 HDFS 文件 → SerDe 反序列化成行<br>2) 执行 `WHERE age > 30` 过滤<br>3) 选取 `dept` 列<br>4) 按 `dept` 做**局部聚合**（Combiner 思路）<br>5) 输出 `<dept, partial_count>` |
| **Shuffle**      | 按 `dept` 的 hash 把相同 key 路由到同一 Reducer                                                                                                             |
| **Reduce 任务**    | 对相同 `dept` 的 partial_count 做**最终求和**                                                                                                              |
| **OutputFormat** | 把结果写回 HDFS（默认 `/tmp/hive/...` 或目标表路径）                                                                                                             |

---

### 4.1.3 Hive 操作 HDFS 文件的"桥梁"

这里你提到的元数据是关键，详细展开一下 **Metastore 里到底存了什么**：

| 元数据类别     | 存的内容                        | 例子                                                                      |
| --------- | --------------------------- | ----------------------------------------------------------------------- |
| **库/表信息** | DB 名、表名、Owner、创建时间          | `default.employee`                                                      |
| **列信息**   | 字段名、类型、注释                   | `id INT, name STRING, dept STRING, age INT`                             |
| **存储信息**  | HDFS 路径、文件格式、SerDe 类、压缩方式   | `location='/user/hive/warehouse/employee'`，`InputFormat=OrcInputFormat` |
| **分区信息**  | 分区列、各分区对应的子目录               | `dt='2026-05-07'` → `/.../employee/dt=2026-05-07/`                      |
| **统计信息**  | 行数、文件大小、字段 NDV 等（用于 CBO 优化） | numRows=1000000                                                         |

> 🔑 **正是因为这套映射关系**，Hive 才能让一张"逻辑表"对应到 HDFS 上的一组目录/文件。**删除 Hive 表本质就是删元数据 + 删 HDFS 目录**（外部表只删元数据）。
...
```
   用户写：SELECT dept, COUNT(*) FROM employee WHERE age>30 GROUP BY dept;
                        │
                        ▼
        ┌───────────────────────────────────┐
        │  ① Parser: SQL → AST              │
        │  ② Analyzer: 查 Metastore 校验    │──查询──► [Metastore: 表→HDFS路径映射]
        │  ③ LogicalPlan: OperatorTree      │
        │  ④ Optimizer: 谓词下推/列裁剪等   │
        │  ⑤ PhysicalPlan: 翻译成 MR Task   │
        └───────────────────────────────────┘
                        │
                        ▼ 提交 Job
              ┌──────────────────────┐
              ...
              ▼                   ▼
        ┌──────────┐         ┌──────────┐
        │ Map Task │ ─────► │ Reduce   │ ─► HDFS 输出
        │ 读HDFS   │ Shuffle │ Task    │
        └──────────┘         └──────────┘
              │
              ▼
       /user/hive/warehouse/employee/*.orc  ← Metastore 告诉它去哪读
```
...
1. **Hive 不一定跑 MapReduce**：现在生产里更多用 `hive.execution.engine=tez` 或 `spark`，编译流程前 4 步几乎一样，只是第 5 步把 OperatorTree 翻译成 **Tez DAG** 或 **Spark Stage**，比 MR 快很多（避免反复落盘）。

2. **EXPLAIN 是利器**：你可以执行 `EXPLAIN SELECT ...` 直接看 Hive 把你的 SQL 翻译成了哪些 Stage、每个 Stage 是 MapRedTask 还是 MoveTask，对调优极有帮助。

3. **和 Spark SQL 的对比**：Spark SQL 走的是 **Catalyst 优化器 + Tungsten 执行引擎**，逻辑上和 Hive 编译流程类似（Parser→Analyzer→Optimizer→PhysicalPlan），但优化能力更强、还支持代码生成（WholeStageCodegen），所以同一条 SQL 通常 Spark SQL 比 Hive on MR 快一个数量级。

---

## 要不要我再展开讲：

- 🔍 **`EXPLAIN` 输出怎么读**？带一个真实例子帮你解析每个 Stage？
- ⚙️ **Hive 的常见物理优化**（MapJoin、SMB Join、Bucket Join、动态分区裁剪）的原理？
- 🆚 **Hive on MR vs Hive on Tez vs Spark SQL** 在编译/执行机制上的差异？

你最想深入哪一块，我可以接着画图拆给你看 👇
