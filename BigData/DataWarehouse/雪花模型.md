...
```sql
-- 类目（树形）
INSERT INTO ods_category PARTITION(dt='2026-05-11') VALUES
('1','食品',NULL),
('2','饮料','1'),
('3','碳酸饮料','2'),
('4','可乐','3'),
...
-- 品牌
INSERT INTO ods_brand PARTITION(dt='2026-05-11') VALUES
('B01','可口可乐公司'),
('B02','百事公司');

-- 商品
INSERT INTO ods_product PARTITION(dt='2026-05-11') VALUES
('P001','可口可乐330ml','5','B01',3.50),
('P002','百事可乐330ml','4','B02',3.20);

-- 订单
INSERT INTO ods_order PARTITION(dt='2026-05-11') VALUES
('O1001','U1','P001',7.00 ,'2026-05-11 10:00:00','I'),
('O1002','U2','P002',6.40 ,'2026-05-11 11:00:00','I'),
('O1003','U3','P001',10.50,'2026-05-11 12:00:00','I');
```
...
CREATE TABLE dwd_dim_product (
  product_id    STRING,
  product_name  STRING,
  category_id   STRING,   -- ❄️ 外键
  brand_id      STRING,   -- ❄️ 外键
  price         DECIMAL(12,2)
);
```
...
```sql
CREATE TABLE dwd_dim_category (
  category_id          STRING,
  category_name        STRING,
  parent_category_id   STRING   -- ❄️ 指向自己
);
```
...
INSERT OVERWRITE TABLE dwd_order_fact PARTITION(dt='2026-05-11')  
...
SELECT product_id, product_name, category_id, brand_id, price  
...
INSERT OVERWRITE TABLE dwd_dim_category  
...
INSERT OVERWRITE TABLE dwd_dim_brand  
...
dwd_dim_category ❄️ dwd_dim_brand  
...
雪花结构虽然规范，但 **每次查询都要递归 + 多表 join**，不适合直接给报表。
...
CREATE TABLE dws_dim_category_flat (  

category_id STRING,  

lvl1_name STRING, -- 一级类目  
...
INSERT OVERWRITE TABLE dws_dim_category_flat  
...
CONCAT(t.path,'>',c.category_name),  

t.lvl + 1  

FROM dwd_dim_category c  
...
SPLIT(path,'>')[0] AS lvl1_name,  

SPLIT(path,'>')[1] AS lvl2_name,  

SPLIT(path,'>')[2] AS lvl3_name,  

SPLIT(path,'>')[3] AS lvl4_name,  

category_name AS leaf_name  

FROM cat_tree;  

显示更多行

👉 **这一步是工程中最关键的“雪花变星型”操作** ✅
...
CREATE TABLE dws_dim_product_wide AS  
...
c.leaf_name AS category_name  

FROM dwd_dim_product p  

LEFT JOIN dwd_dim_brand b  

ON p.brand_id = b.brand_id  

LEFT JOIN dws_dim_category_flat c  
...
INSERT OVERWRITE TABLE dws_sales_day  

SELECT  

date_format(f.order_time,'yyyy-MM-dd') AS dt,  
...
JOIN dws_dim_product_wide w  

ON f.product_id = w.product_id  

GROUP BY  

date_format(f.order_time,'yyyy-MM-dd'),  

w.lvl1_name, w.lvl2_name, w.brand_name;  
...
> **DWS 把雪花维度展平为星型宽表，并提前算好通用汇总，提升查询性能。** ⭐

---

# 五、ADS 层：面向应用

ADS 是最“贴业务”的一层，**报表 / 接口 / BI 直接用**。
...
CREATE TABLE ads_sales_lvl1_daily AS  
...
GROUP BY dt, category_lvl1;  

显示更多行

---

## ✅ 2. 报表示例：可口可乐 vs 百事可乐对比

SQL

CREATE TABLE ads_brand_battle_daily AS  
...
WHERE brand_name IN ('可口可乐公司','百事公司')  

GROUP BY dt, brand_name;  
...
> **ADS 是数仓对业务的最终交付层，只面向场景，不再做复杂加工。**
...
dws_dim_category_flat ← 类目展平  

dws_dim_product_wide ← 商品宽表  
...
ads_sales_lvl1_daily  

ads_brand_battle_daily  
...
> **ODS 保留原始数据，DWD 用雪花模型规范化维度并保留业务明细，DWS 将雪花维度展平为星型宽表并预计算指标，ADS 面向具体业务输出最终结果。  
> 这是工程中“规范性 + 性能”兼顾的标准做法。**

---

如果你愿意，下一步我可以继续给你做：

- ✅ 把这套示例**改造成你 TCEC / Billing / Finance 项目**的真实版本
- ✅ 给你写一份 **ETL 调度顺序 + 依赖图（Airflow / ADF 风格）**
- ✅ 出一套 **面试问答：“你们项目是雪花还是星型？怎么选的？”**
