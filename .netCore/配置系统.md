...
开发Web.config提供者1、.NET Core中不建议使用.NET Framework里的web.config，不过仍然继续提供了ConfigurationManager了，不过没有官方支持通过新的Configuration框架读取的方式。我们来实现一个，能够读取web.config里的connectionstrings和appsettings的内容。没用过web.config也没关系。